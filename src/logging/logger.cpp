...
Logger::Logger()
    ...
    , m_maxBackupFiles(5)
    , m_logLevel(Debug)  // Default to Debug, can be changed later via setLogLevel()
    , m_consoleOutput(true)
{
    // Default log path: AppDataLocation/logs/pix3lforge.log
...
void Logger::initialize(const QString& logFilePath, qint64 maxFileSize, int maxBackupFiles)
{
    ...
    if (!logDir.exists()) {
        logDir.mkpath(".");
        // Log directory created (no qDebug to avoid recursion with message handler)
    }

    // Open log file in append mode
    m_logFile.setFileName(m_logFilePath);
    if (!m_logFile.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) {
        // Failed to open log file (no qCritical to avoid recursion with message handler)
        return;
    }
    ...
    // Logger initialized (no qDebug to avoid recursion with message handler)
}
...
void Logger::setLogLevel(LogLevel level)
{
    QMutexLocker locker(&m_mutex);
    m_logLevel = level;
    // Log level set (no qDebug to avoid recursion with message handler)
}
...
void Logger::rotateLogFile()
{
    ...
    m_logFile.setFileName(m_logFilePath);
    if (!m_logFile.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) {
        // Failed to reopen log file after rotation (no qCritical to avoid recursion)
        return;
    }
    ...
    // Log file rotated (no qDebug to avoid recursion with message handler)
}
...
int Logger::clearLogs()
{
    ...
    // Logs cleared (no qDebug to avoid deadlock with message handler and mutex)
    return deletedCount;
}
